...
# 2. GÜVENLİ VE DÜZELTİLMİŞ BAĞLANTI
@st.cache_resource
def baglan():
    scope = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
    
    # Secrets'tan gelen veriyi bir sözlük (dictionary) olarak kopyalıyoruz
    gizli_bilgiler = dict(st.secrets["gcp_service_account"])
    
    # İŞTE HAYAT KURTARAN SATIR: Şifrenin içindeki bozuk \n karakterlerini gerçek alt satıra çeviriyoruz
    gizli_bilgiler["private_key"] = gizli_bilgiler["private_key"].replace("\\n", "\n")
    
    # Temizlenmiş şifreyle Google'a bağlanıyoruz
    creds = Credentials.from_service_account_info(gizli_bilgiler, scopes=scope)
...
# --- BURADAN SONRASI KAYIT VE ANALİZ FORMLARI ---
